...
<script>
    ...
    function initImageUploader() {
        $('#drag-and-drop-zone').dmUploader({
            url: '<?= base_url('File/uploadImageSession'); ?>',
            ...
            onUploadSuccess: function (id, data) {
                loadImagePreview(id, data.file_id);
            },
    ...
    function loadImagePreview(id, fileId) {
        $.ajax({
            type: 'POST',
            url: generateUrl('File/getSessUploadedImage'),
            data: {'file_id': fileId},
            success: function (response) {
                if (response.result == 1) {
                    document.getElementById("uploaderFile" + id).innerHTML = response.imageHtml;
                }
            }
        });
    }
